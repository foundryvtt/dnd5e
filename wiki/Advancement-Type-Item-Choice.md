...
If the "Item Type" is set to "Spell" then an additional set of controls appear. The "Spell Level" allows for restricting what levels of spells can be chosen when added manually by the player. It can be restricted to a specific spell level or allow for selecting any spell that the character can cast. The "Spell Schools" and "Spell Lists" allows restricting the spell selection to only spells belonging to certain schools or included on certain spell lists. The remaining controls affect the spells that are added to the sheet. More information on using these controls is available on the [Grant Items advancement guide](Advancement-Type-Grant-Items).
