  ...
  *allApplicableEffects() {
  ...
  static prepareSpellcastingSlots(spells, type, progression, {actor}={}) {
    ...
    if ( allowed && CONFIG.DND5E.spellcasting[type].separate ) {
      ...
      this.prepareAltSlots(spells, actor, progression, type);
    }
  }
   ...
   * @param {object} [table]       A progression table to use to determine spell slots, for backwards compatibility.
   */
  static prepareAltSlots(spells, actor, progression, key, table) {
    if ( table ) {
      foundry.utils.logCompatibilityWarning("When preparing spell slots, a table should be registered in DND5E.spellcasting.", {
        since: "5.0", until: "5.4", once: true
      });

      // Workaround for deprecation and assigning table to the spellcasting on i18nInit.
      if ( actor._id === "dnd5eDeprecation" ) actor._table = table;
      return;
    }
    ...
    // Apply slots
    const configuration = CONFIG.DND5E.spellcasting[key].calculateSlots(keyLevel);
    ...
    spells[key].value = Math.clamp(spells[key].value, 0, spells[key].max) || 0;
  }
  ...
  static prepareLeveledSlots(spells, actor, progression, key) {
    const config = CONFIG.DND5E.spellcasting[key];
    const slots = config.calculateSlots(progression[key]);
    for ( const level of Array.fromRange(Object.keys(CONFIG.DND5E.spellLevels).length - 1, 1) ) {
      const slot = spells[config.spellSlotKey(level)] ??= { value: 0 };
      slot.label = config.spellSlotLabel(level);
      slot.level = level;
