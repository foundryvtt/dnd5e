...
async function enrichDamage(config, label, options) {
  ...
  if ( config.average ) {
    ...
    if ( config.average === true ) {
      const minRoll = Roll5e.create(config.formula).evaluate({ minimize: true, async: true });
      const maxRoll = Roll5e.create(config.formula).evaluate({ maximize: true, async: true });
