...
export default class ItemSheet5e extends PrimarySheetMixin(DocumentSheet5e) {
  ...
  /**
   * ProseMirror editors for the sheet.
  ...
  async _preparePartContext(partId, context) {
    ...
    switch ( partId ) {
      case "activities": context = await this._prepareActivitiesContext(context); break;
      case "advancement": context = await this._prepareAdvancementContext(context); break;
      ...
      case "header": context = await this._prepareHeaderContext(context); break;
    }
   ...
   * @protected
   */
  async _prepareActivitiesContext(context) {
   ...
   * @protected
   */
  async _prepareAdvancementContext(context) {
  ...
  /** @override */
  _addDocument() {
    if ( this.tabGroups.sheet === "activities" ) {
