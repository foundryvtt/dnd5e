...
export default class ItemSheet5e extends PrimarySheetMixin(DocumentSheet5e) {
  ...
  async _prepareDetailsContext(context, options) {
    ...
    // If using modern rules, do not show redundant artificer progression, but show any current value.
    context.spellProgression = CONFIG.DND5E.spellcasting.progressionChoices;
    const current = this.item.system.spellcasting?.progression;
    if ( (game.settings.get("dnd5e", "rulesVersion") === "modern") && (current !== "artificer") ) {
      context.spellProgression.findSplice(s => s.value === "artificer");
    }
    if ( current && !context.spellProgression.find(s => s.value === current) ) {
      context.spellProgression.push({ value: current, label: current });
    }
