...
export default class AdvancementFlow extends Application5e {
  constructor(options, advancementId, level, _options={}) {
    if ( options instanceof Item ) {
      ...
      options = { advancement: options.advancement?.byId[advancementId], level, ..._options };
    }
    super(options);
    this.#advancementId = this.options.advancement.id;
    this.#item = this.options.advancement.item;
  }
   ...
   */
  get advancement() {
    return this.item.advancement?.byId[this.#advancementId] ?? null;
  }
  ...
  _initializeApplicationOptions(options) {
    ...
    options.uniqueId = `actor-${options.advancement.item.id}-advancement-${options.advancement.id}-${options.level}`;
    return options;
  }
