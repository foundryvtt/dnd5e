...
export default class InventoryElement extends HTMLElement {
  ...
  _getContextOptions(item, element) {
    ...
    options.push({
      ...
      name: `DND5E.ContextMenuAction${item.system.preparation?.prepared ? "Prepare" : "Unprepare"}`,
      icon: '<i class="fa-solid fa-sun fa-fw"></i>',
      condition: () => {
        const isPrepared = ("preparation" in item.system) && (item.system.preparation.prepared < 2);
