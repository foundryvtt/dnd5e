...
export default class CharacterActorSheet extends BaseActorSheet {
  ...
  async _getFavoriteData(type, id) {
    ...
    if ( type === "slots" ) {
      ...
      if ( !config?.separate ) return {
        uses, level,
        title: game.i18n.localize(`DND5E.SpellSlots${id.capitalize()}`),
        subtitle: [
          game.i18n.localize(`DND5E.SpellLevel${level}`),
          game.i18n.localize(`DND5E.Abbreviation${CONFIG.DND5E.spellcasting[id]?.isSR ? "SR" : "LR"}`)
        ],
        img: CONFIG.DND5E.spellcasting[id]?.img || CONFIG.DND5E.spellcasting.pact.img
      };
      ...
      return {
        ...
        subtitle: game.i18n.localize(`DND5E.Abbreviation${config.isSR ? "SR" : "LR"}`),
        img: config.img.replace("{id}", id)
      };
    }
