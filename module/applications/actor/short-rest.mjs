...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/shaper/templates/apps/short-rest.hbs",
      classes: ["shaper", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("shaper", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("SHAPER.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("SHAPER.Rest"),
            callback: html => {
              let newDay = false;
              if ( game.settings.get("shaper", "restVariant") !== "epic" ) {
