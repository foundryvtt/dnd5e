...
export default class BaseActorSheet extends PrimarySheetMixin(
  ...
  _prepareSpellbook(context) {
    ...
    const progressions = Object.values(CONFIG.DND5E.spellcasting).filter(s => !s.isStatic);

    /**
     * Register a section in the spellbook.
     * @param {string} sectionKey             The key for sorting spells ('item', 'atwill', 'spell3', etc.).
     * @param {number} [spellLevel]           The level of spells in this section. Only relevant for `separate`
     *                                        progressions.
     * @param {SpellcastingModel} [config]    A spellcasting model this belongs to. Can be skipped if "item".
     */
    const registerSection = (sectionKey, spellLevel=null, config=null) => {
      // Section used by spells when organizing.
      sectionKey = (sectionKey === "item") ? sectionKey : config.spellSlotKey(spellLevel);

      // Prevent double-registering cantrips section.
      if ( spellbook[sectionKey] ) return;

      // Displayed label on the section in the spellbook tab.
      const label = (sectionKey === "item") ? game.i18n.localize("DND5E.CAST.SECTIONS.Spellbook") : config.spellSlotLabel(spellLevel);

      // The spell preparation mode (if it matters).
      const slot = (sectionKey === "item") ? sectionKey : config.key;

      // The spell level (if it matters).
      const level = (sectionKey === "item") || config.isStatic ? 1 : spellLevel;

      // The order this section is displayed in.
      const order = (sectionKey === "item") ? 0 : config.order + level;

      // Are spell slots displayed on this section?
      const usesSlots = (sectionKey === "item") ? false : !!spellLevel && !config.isStatic;

      const items = [];
      const section = spellbook[sectionKey] = {
        label, slot: sectionKey, columns, order, draggable: true, usesSlots, id: slot, // FIXME
        dataset: { type: "spell", level, preparationMode: slot },
        items, spells: items, // FIXME
        minWidth: 220
      };
      if ( !section.usesSlots ) return;

      const spells = foundry.utils.getProperty(this.actor.system.spells, sectionKey);
    ...
    // Set up initial sections proper spellcasting progressions.
    for ( const prog of progressions ) {
      const levels = prog.spellSections(this.actor);
      if ( !levels.length ) continue;
      if ( !spellbook.spell0 && prog.cantrips ) registerSection("spell0", 0, CONFIG.DND5E.spellcasting.spell);
      if ( prog.separate ) {
        for ( let lvl = 1; lvl <= Math.max(...levels); lvl++ ) {
          registerSection(prog.spellSlotKey(lvl), lvl, prog);
        }
      } else {
        const [lvl] = levels;
        registerSection(prog.spellSlotKey(lvl), lvl, prog);
    ...
    (context.itemCategories.spells ?? []).forEach(spell => {
      let mode = spell.system.preparation.mode;
      if ( !(mode in CONFIG.DND5E.spellcasting) ) mode = "atwill";

      const config = CONFIG.DND5E.spellcasting[mode];
      const spellLevel = spell.system.level || 0;
      let sectionKey = config.spellSlotKey(spellLevel);

      // Spells from items
      if ( spell.getFlag("dnd5e", "cachedFor") ) {
        sectionKey = "item";
        if ( !spell.system.linkedActivity?.displayInSpellbook ) return;
        if ( !spellbook[sectionKey] ) {
          registerSection(sectionKey);
          spellbook[sectionKey].order = 10000;
        }
      }

      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
      else if ( !spellbook[sectionKey] ) {
        registerSection(sectionKey, spellLevel, config);
      }

      // Add the spell to the relevant heading
      spellbook[sectionKey].spells.push(spell);
    });
  ...
  async _prepareItemSpell(item, ctx) {
    ...
    if ( config?.prepares && !linked ) {
      const isAlways = item.system.preparation?.prepared === CONFIG.DND5E.spellPreparationStates.always.value;
  ...
  _filterItems(items, filters) {
    ...
    return items.filter(item => {
      ...
      if ( filters.has("prepared") ) {
        const isStatic = CONFIG.DND5E.spellcasting[item.system.preparation?.mode]?.isStatic;
        return isStatic || (item.system.preparation?.prepared > 0);
      }
