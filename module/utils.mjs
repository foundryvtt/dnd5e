 ...
 * (e.g. "shaper.actor-traits").
 * @returns {Promise}
 */
export async function preloadHandlebarsTemplates() {
  const partials = [
    // Shared Partials
    "systems/shaper/templates/actors/parts/active-effects.hbs",
  ...
  for ( const path of partials ) {
    ...
    paths[`shaper.${path.split("/").pop().replace(".hbs", "")}`] = path;
  }
...
export function registerHandlebarsHelpers() {
  Handlebars.registerHelper({
    ...
    "shaper-linkForUuid": linkForUuid
  });
}
 ...
 * @param {string} configKey              Key within `CONFIG.SHAPER` to localize.
 ...
 * @param {object} config  The `CONFIG.SHAPER` object to localize and sort. *Will be mutated.*
 */
