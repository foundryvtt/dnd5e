# Contributing to foundrynet/shaper
...
Cloning this repository and either placing it in or symlinking it to your `Data/systems/shaper` user data directory is all that is necessary to run this within Foundry VTT. However, if you want to make changes to either the LESS stylesheets or the compendia, there are some developer tools which will make your life easier.
...
Converts the LESS in `./less` to the final `shaper.css`.
...
Non-English languages are not contained within the core shaper system, but instead they are managed by specialized [localization modules](https://foundryvtt.com/packages/tag/translation).
...
```text
https://github.com/foundryvtt/shaper/releases/download/release-1.6.3/shaper-1.6.3.zip
                                                     └─ Tag Name ──┘     └─ V ─┘ (version)
```
